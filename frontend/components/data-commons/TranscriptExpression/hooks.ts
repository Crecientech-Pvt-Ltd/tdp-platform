...
export function useSampleMapping(sampleData: string | undefined) {
  ...
  const getCachedNormalizedSample = useCallback((sample: string): string => {
  ...
  useEffect(() => {
    ...
    if (!groupColumn) {
      setGroupColumn(fields[fields.length - 1] === "" || fields[fields.length - 1] === undefined ? `col_${fields.length - 1}` : fields[fields.length - 1])
    }
...
export function useGeneDataMap(
  ...
  const getSampleGroup = useCallback((sample: string): string => {
  ...
  const groupAndSortSamples = useCallback((samples: string[]): { samples: string[], groups: string[] } => {
    if (!sampleDataExists) {
  ...
  const getBarColors = useCallback((x: string[]) => {
    if (!sampleDataExists) {
      return x.map(() => "#6b7280")
    }
