...
const VirtualizedCommand = ({
	...
	const [filteredOptions, setFilteredOptions] =
		React.useState<(string | GenePropertyMetadata)[]>(options);
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
								...
								onClick={() =>
									onSelectOption?.(
										filteredOptions.slice(0, 50).map(getProperty),
									)
								}
							>
						...
						<TooltipContent>
							Select all (only first 50 items are selected at max)
						</TooltipContent>
					</Tooltip>
				)}
			</CommandInput>
		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
					...
					aria-expanded={open}
					className={cn(
						"w-[200px] justify-between text-ellipsis text-wrap break-words h-9",
						className,
					)}
				>
