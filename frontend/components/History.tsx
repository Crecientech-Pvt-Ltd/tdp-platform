	...
	return (
		<div className="h-[92%]">
			...
			{history.length > 0 ? (
							...
							<Card key={`${item.title}-${item.createdAt ?? index}`}>
								<CardHeader className="p-2">
									...
									<div className="pl-1 text-xs text-muted-foreground">
										<p>
											{item.seedGenes.slice(0, 30) +
												(item.seedGenes.length > 30 && "...")}
										</p>
										<p>
											{item.diseaseMap.split(" ").at(-1)?.slice(1, -1)} : Order
											- {item.order} : {item.interactionType} : {item.minScore}
										</p>
									</div>
								</CardHeader>
				...
				<p className="text-center h-full italic grid place-items-center text-lg">
