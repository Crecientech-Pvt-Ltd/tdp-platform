...
export function GraphEvents({
	...
	useEffect(() => {
		...
		for (const node of geneNames) {
			if (
				previousHighlightedNodes.has(node) ||
				!graph.hasNode(node) ||
				graph.getNodeAttribute(node, "hidden") === true
			)
				continue;
	...
	useEffect(() => {
		...
		const suggestions = trieRef.current
		...
		registerEvents({
			...
			clickNode: (e) => {
				...
				if (e.event.original.ctrlKey) {
					...
					const appendedQuery = trimmedQuery
						? trimmedQuery.replace(/[,\s]*$/, `, ${node},`)
						: `${node},`;
	...
	const selectedNodeSizeProperty = useStore(
		(state) => state.selectedNodeSizeProperty,
	);
	const selectedRadioNodeSize = useStore(
		(state) => state.selectedRadioNodeSize,
	);
	...
	const propertyResolve = useCallback(
		(
			node: string,
			selectedRadio: NodeColorType | NodeSizeType | undefined,
			selectedProperty: string | Set<string>,
		) => {
			if (!selectedRadio || !selectedProperty) return <></>;
			const diseaseNameOrCommon = DISEASE_DEPENDENT_PROPERTIES?.includes(
			...
			if (typeof selectedProperty === "string") {
				const value = (
					universalData[node]?.[
						userRadioArr?.includes(selectedProperty)
							? "user"
							: diseaseNameOrCommon
			...
			const values = selectedProperty.size
				? Array.from(selectedProperty).map((prop) => {
						const value = (
							universalData[node]?.[
								userRadioArr?.includes(prop) ? "user" : diseaseNameOrCommon
							] as OtherSection & CommonSection
						)?.[selectedRadio]?.[prop];
						return (
							<div key={prop}>
			...
			{clickedNode && (
				<div className="absolute top-0 right-0 space-y-1 text-xs shadow rounded border backdrop-blur p-1 m-1 w-80 max-h-[80vh] overflow-y-auto">
					...
					<div>
						...
						<p>
							{clickedNode
								? sigma.getGraph().getNodeAttribute(clickedNode, "label")
								: "No Node Selected"}
						</p>
					</div>
					<div>
						...
						<p>
							{clickedNode
								? sigma.getGraph().getNodeAttribute(clickedNode, "description")
								: "No Node Selected"}
						</p>
					</div>
					{propertyResolve(
