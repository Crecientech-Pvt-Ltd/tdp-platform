...
export function FileSheet() {
		...
		openDB("files", "readonly").then((store) => {
			if (!store) {
				toast.error("Failed to open IndexedDB database", {
					...
					description:
						"Please make sure you have enabled IndexedDB in your browser",
				});
				return;
			}
	...
	const removeFile = async (name?: string) => {
		setUploadedFiles(
			name ? uploadedFiles.filter((file) => file.name !== name) : [],
		);
			...
			request.onsuccess = async () => {
				...
				try {
					for (const row of parsedData.data) {
						...
						const geneID = firstValue.startsWith("ENSG")
							? firstValue
							: geneNameToID.get(firstValue?.toUpperCase());
						...
						for (const prop in row) {
							...
							for (const field of [
								...
								if (fieldRegex.test(prop)) {
									universalData[geneID].user[field][
										prop.replace(fieldRegex, "")
									] = row[prop];
									break;
								}
							}
	...
	return (
		<div>
							...
							<AlertDialog open={showConfirmDialog}>
								<AlertDialogContent>
									...
									<AlertDialogFooter>
										...
										<AlertDialogAction onClick={() => removeFile()}>
											Continue
										</AlertDialogAction>
									</AlertDialogFooter>
								</AlertDialogContent>
			...
			<div className="text-xs text-gray-500 italic mt-2">
				<b>NOTE:</b> The uploaded files will be stored in your browser's local
				storage and is not shared with anyone.
			</div>
		</div>
	);
}
