...
Now we focus on the functions of network visualization board. You can **hover** on this icon <Image width={30} height={30} src="/image/docs/help-icon.png" alt="Help Icon" className="inline aspect-square"/> on the top left for a quick help of shortcuts.
...
2. Click "Export" button and select proper format. You can export the network in two formats: **CSV** and **PNG**.
   - CSV: This is a comma-separated-values file that contains the network data, which can be used for further analysis or visualization in other tools. We offer two types of CSV files:
     - Universal: This file contains the nodes (genes) of the network, including Ensembl ID, gene name, and gene description.
        * If you have mapped the node color or size to any property, those properties will also be included in this file.
        * If some nodes are highlighted or selected as hub genes, only those nodes will be included in this file.
...
You can visualize the OpenTargets heatmap for the selected genes on the network visualization board. We offer both Target-disease Association and Target prioritization factors.

![Target-disease Association & Target prioritization factors](../../public/image/docs/network-visualization-association-factors.png)

1. Search for your genes of interest in the search box.

![Search for genes](../../public/image/docs/network-visualization-genes.png)

2. Click on different categories to sort the genes/targets in the heatmap.

![Sort by category](../../public/image/docs/network-visualization-sort.png)