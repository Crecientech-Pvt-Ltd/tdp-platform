...
DISEASE_DEPENDENT_FIELDS = ["LogFC", "OpenTargets", "Genetics"]
...
class ClickHouseSeeder:
    ...
    def refresh_materialized_view(self, view_name: str, wait: bool = True) -> None:
        """
        Manually refresh a materialized view
        
        Args:
            view_name: Name of the materialized view to refresh
            wait: Whether to wait for the refresh to complete
        """
        try:
            # Start the view (enable scheduling)
            self.client.command(f"SYSTEM START VIEW {view_name}")
            console.print(f"[blue]Refreshing materialized view: {view_name}...[/blue]")
            
            # Trigger immediate refresh
            self.client.command(f"SYSTEM REFRESH VIEW {view_name}")
            
            # Optionally wait for completion
            if wait:
                self.client.command(f"SYSTEM WAIT VIEW {view_name}")
                console.print(f"[green]✓ Materialized view {view_name} refreshed successfully[/green]")
            
            # Stop the view (disable automatic refreshes)
            self.client.command(f"SYSTEM STOP VIEW {view_name}")
        except Exception as e:
            console.print(f"[yellow]Warning: Failed to refresh view {view_name}: {e}[/yellow]")
...
def detect_format(df: pd.DataFrame, id_column: str) -> str:
    ...
    for col in columns:
        if len(col.split("_LogFC_")) >= 2 or len(col.split("_OpenTargets_")) >= 2 or len(col.split("_Genetics_")) >= 2:
            has_prefixes = True
            break
...
def parse_column_mapping(
    ...
    for col in columns:
        ...
        # Check for disease-specific patterns: <disease>_OpenTargets_<property>, <disease>_LogFC_<property>, or <disease>_Genetics_<property>
        if "_OpenTargets_" in col:
            ...
            parts = col.split("_LogFC_")
            if len(parts) == 2:
                disease_id = parts[0]
                property_name = parts[1]
                table_name = "differential_expression"
        elif "_Genetics_" in col:
            parts = col.split("_Genetics_")
            if len(parts) == 2:
                disease_id = parts[0]
                property_name = parts[1]
                table_name = "genetics"
    ...
    if table_name == "differential_expression":
        if "disease_id" not in processed_df.columns:
            ...
            if len(split_parts.columns) == 2:
                ...
                console.print(
                    f"[red]Error: property_name column does not contain expected '_LogFC_' pattern for {table_name} table, when disease_id column is not available[/red]"
                )
                sys.exit(1)
        else:
            # Ensure property_name is correctly set
            processed_df["property_name"] = (
                processed_df["property_name"].str.split("_LogFC_").str[-1]
            )

    if table_name == "genetics":
        if "disease_id" not in processed_df.columns:
            # Split once and assign both parts efficiently
            split_parts = processed_df["property_name"].str.split(
                "_Genetics_", n=1, expand=True
            )
            if len(split_parts.columns) == 2:
                processed_df["disease_id"] = split_parts[0]
                processed_df["property_name"] = split_parts[1]
                ...
                console.print(
                    f"[red]Error: property_name column does not contain expected '_Genetics_' pattern for {table_name} table, when disease_id column is not available[/red]"
                )
                sys.exit(1)
        else:
            # Ensure property_name is correctly set
            processed_df["property_name"] = (
                processed_df["property_name"].str.split("_Genetics_").str[-1]
            )
            ...
            if table_choice == "overall_association_score":
                ...
                if prop_answers:
                    mapping[col] = {
                        ...
                        "requires_disease": table_choice in ["differential_expression", "genetics"],
...
def seed(
    ...
    Example columns: Pathway_<name>, Druggability_<name>, TE_<name>, OT_Prioritization_<name>, <disease>_LogFC_<name>, <disease>_OpenTargets_<datasource>, <disease>_Genetics_<name>
    ...
    Example rows:
        ...
        gene1, <disease>_LogFC_LogFC, 2.5
        OR
        gene1, LogFC, 2.5, <disease>
        --------------------------------
        # genetics table
        gene1, <disease>_Genetics_L2G, 0.8
        OR
        gene1, L2G, 0.8, <disease>
        --------------------------------
    ...
    # The mv_datasource_association_score_overall_association_score view depends on:
    # - datasource_association_score
    # - overall_association_score
    ...
    inserted_tables = set(table_dataframes.keys())
    if inserted_tables & tables_requiring_mv_refresh:
        console.print(
            "\n[blue]Refreshing dependent materialized views...[/blue]"
        )
        clickhouse_seeder.refresh_materialized_view(
