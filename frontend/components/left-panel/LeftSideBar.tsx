...
export function LeftSideBar() {
	...
	const [fetchHeader, { loading, called }] = useLazyQuery<
		GetHeadersData,
		GetHeadersVariables
	>(GET_HEADERS_QUERY(bringCommon.current), { returnPartialData: true });
					...
					for (const field of DISEASE_DEPENDENT_PROPERTIES) {
						if (new RegExp(`^${diseaseName}_${field}_`, "i").test(name)) {
							radioOptions.database[field].push({
								description,
								name: name.replace(
									new RegExp(`^${diseaseName}_${field}_`, "i"),
									"",
								),
							});
						}
	...
	const [fetchUniversal, { loading: universalLoading }] = useLazyQuery<
		GeneUniversalData,
		GeneUniversalDataVariables
	...
	async function handlePropChange(
		...
		const selectedRadio =
			type === "color" ? selectedRadioNodeColor : selectedRadioNodeSize;
		if (!selectedRadio) return;
		const ddp = DISEASE_DEPENDENT_PROPERTIES.includes(
			selectedRadio as DiseaseDependentProperties,
		);
		const keys = (val instanceof Set ? Array.from(val) : [val]).reduce<
			string[]
		>((acc, v) => {
			const key = `${ddp ? `${diseaseName}_` : ""}${selectedRadio}_${v}`;
			if (
				!queriedFieldSet.current.has(key) &&
				!radioOptions.user[selectedRadio].includes(key)
			) {
		...
		if (keys.length === 0) {
			...
			for (const gene of data ?? []) {
				for (const prop in gene.common) {
					universalData[gene.ID].common[
						selectedRadio as DiseaseIndependentProperties
				...
				for (const prop in gene.disease?.[diseaseName]) {
					...
					(universalData[gene.ID][diseaseName] as OtherSection)[
						selectedRadio as DiseaseDependentProperties
					][prop.replace(new RegExp(`^${selectedRadio}_`), "")] =
				...
				<div className="flex items-center gap-2">
					...
					>
						<Combobox
							...
							onChange={(value) =>
								typeof value === "string" &&
								useStore.setState({ diseaseName: value })
							}
