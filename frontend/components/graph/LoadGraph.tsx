...
export function LoadGraph() {
	...
	const [fetchData, { data: response, loading, error }] = useLazyQuery<
		GeneGraphData,
		GeneGraphVariables
	>(GENE_GRAPH_QUERY, {
		variables: {
	...
	const [fetchFileData] = useLazyQuery<
		GeneVerificationData,
		GeneVerificationVariables
			...
			if (fileName) {
				...
				req.onsuccess = async () => {
					...
					if (fileType === "json") {
						...
						fileData = parsedResult.data as Array<
					...
					if (fields.length < 3) {
						toast.error("There must be atleast 3 fields in csv/json!", {
							description:
								"Fields more than 3 are ignored. Please check the file and try again",
							cancel: {
								label: "Close",
								onClick: () => window.close(),
							},
				...
				if (response) {
					...
					if (genes.length > 5000 || links.length > 50000) {
						toast.warning("Large graph detected!", {
							description:
								"Computation is stopped. Auto closing the graph in 3 seconds to prevent browser crash",
							cancel: {
								label: "Close",
								onClick: () => window.close(),
							},
					...
					localStorage.setItem(
						"graphConfig",
						JSON.stringify({ ...variable, graphName }),
					);
					useStore.setState({ graphConfig: { ...variable, graphName } });
					const transformedData: Partial<
						SerializedGraph<NodeAttributes, EdgeAttributes>
					> = {
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription className="text-black">
								You are about to generate a graph with a large number of
								nodes/edges. You may face difficulties in analyzing the graph.
							</AlertDialogDescription>
