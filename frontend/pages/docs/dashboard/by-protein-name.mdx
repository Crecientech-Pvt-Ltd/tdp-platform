...
You can autofill seed genes, manually type into the "Seed Genes" textbox, or upload a txt file.

- **Autofill seed genes**: You can simply switch the toggle and enter the number of seed genes you want. Then you will see the seed genes with your desired number are automatically filled in the textbox.
- ![Autofill seed genes](../../../public/image/docs/auto-fill-seed-genes.png)
...
The seed genes are the foundation of the network generation, your network will be expanded based on the seed genes you provided. Once you determine the seed genes, you can proceed to [`Step 2`](by-protein-name.mdx#clicking-submit-button) to generate your own network.
...
- **Disease Map**: You can determine your network falls into which disease category, and no worries, if you want to change the disease after generating the network, you can easily change it on the [left panel](../network-visualization/left-panel.mdx) of the network visualization page. We offer over 20,000 different diseases, all from Open Targets Platform. You can simply type in the disease name to search for your desired disease.
...
- **Interaction Type**: We provide 3 different interaction types for you to construct your own network, you can select one or more interaction types to make your network more comprehensive.
  - STRING: Protein-protein interactions from STRING database.
