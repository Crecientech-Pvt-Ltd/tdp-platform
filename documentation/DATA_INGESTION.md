...
- Install Python dependencies with pip command `pip install -r requirements.txt`
...
- The universal data should be in the form of a CSV file. We expect headers in the CSV file in the following order:
  ...
  - Now, other columns can be any property of the gene. It'll be in a matrix form. So, column name should be the of the form `<PropertyType>_<PropertyName>`. Here, `<PropertyType>` can be any of the following:
    ...
    - `<DiseaseId>_LogFC_<PropertyName>`: Differential Expression Gene
    - `<DiseaseId>_OpenTargets_<PropertyName>`: OpenTargets Target Disease Association
    - `<DiseaseId>_Genetics_<PropertyName>`: Genetics Evidence from OpenTargets
...
```csv
ID,TE_prop1,Pathway_prop2,Druggability_prop3,OT_Prioritization_prop4,LogFC_prop5,OpenTargets_prop6
ENSG00000135823,0.5,0.2,0.1,0.3,0.4,0.6
ENSG00000135824,0.6,0.3,0.2,0.4,0.5,0.7
```

```csv
Gene ID,TE_prop1,Pathway_prop2,Druggability_prop3,OT_Prioritization_prop4,LogFC_prop5,OpenTargets_prop6
...
# Data Commons Management

Data Commons provides access to raw data files through the API.

## Directory Structure

```
backend/src/data-commons/data/
├── group1/
│   ├── program1/
│   │   ├── project1/
│   │   │   └── data_file.csv
...
## Adding Data Commons Files

1. **Create directory structure:**

```bash
mkdir -p backend/src/data-commons/data/group_name/program_name/project_name
```

2. **Place data files:**

```bash
cp your_data.csv backend/src/data-commons/data/group_name/program_name/project_name/
```

3. **Add optional password protection:**
- Create a `password.txt` file in the project directory with the desired password.

```bash
echo "your_password" > backend/src/data-commons/data/group_name/program_name/project_name/password.txt
```

## Access Control on Portal

Data Commons uses JWT-based session management:

1. User logs in via frontend
2. JWT token includes validated group/program/project combinations
3. Backend validates access on each request
4. Only validated combinations are accessible


# PPI Network Database Ingestion [Complete Data Building from Scratch]
...
NEO4J_ARGS="-nU bolt://localhost:7687 -nu neo4j -nP $NEO4J_PASSWORD -nd pdnet"
...

{
  ...
  python cli.py universal seed -f data/ot_25.03_datasource_association_score.csv -fmt sparse -t datasource_association_score $CLICKHOUSE_ARGS $NEO4J_ARGS
  python cli.py universal seed -f data/ot_25.03_overall_association_score.csv -fmt sparse -t overall_association_score $CLICKHOUSE_ARGS $NEO4J_ARGS

  sed -i '1s/logFC_\./MONDO_0004976_LogFC_\./g' data/ALS_logFC_from_bill_modified_kept_genes.csv
  python cli.py universal seed -f data/ALS_logFC_from_bill_modified_kept_genes.csv $CLICKHOUSE_ARGS $NEO4J_ARGS
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/Mayo_diagnosis_logFC_transformed_PSP_modified_kept_genes.csv
  ...
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/MSBB_diagnosis_gender_logFC_transformed_PSP_modified_kept_genes.csv
  ...
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/MSBB_diagnosis_gender_logFC_transformed_PSP_modified_kept_genes.csv
  python cli.py universal seed -f data/MSBB_diagnosis_gender_logFC_transformed_PSP_modified_kept_genes.csv -d MONDO_0019037 $CLICKHOUSE_ARGS $NEO4J_ARGS
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/MSBB_diagnosis_logFC_transformed_PSP_modified_kept_genes.csv
  ...
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/ROSMAP_diagnosis_gender_agedeath_logFC_transformed_PSP_modified_kept_genes.csv
  ...
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/ROSMAP_diagnosis_logFC_transformed_PSP_modified_kept_genes.csv
  ...
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/LogFC_PSP_DEGs_Cerebellum_kept_genes_modified.csv
  python cli.py universal seed -f data/LogFC_PSP_DEGs_Cerebellum_kept_genes_modified.csv -d MONDO_0019037 $CLICKHOUSE_ARGS $NEO4J_ARGS
  sed -i '1s/logFC_\./MONDO_0019037_LogFC_\./g' data/LogFC_PSP_DEGs_GP_kept_genes_modified.csv
  python cli.py universal seed -f data/LogFC_PSP_DEGs_GP_kept_genes_modified.csv -d MONDO_0019037 $CLICKHOUSE_ARGS $NEO4J_ARGS

  sed -i '1s/GWAS_\./MONDO_0004976_Genetics_\./g' data/ALS_Genetics_from_bill_modified_kept_genes.csv
  python cli.py universal seed -f data/ALS_Genetics_from_bill_modified_kept_genes.csv -d MONDO_0004976 $CLICKHOUSE_ARGS $NEO4J_ARGS
  sed -i '1s/GWAS_\./MONDO_0019019_Genetics_\./g' data/OI_Genetics_from_bill_modified_kept_genes.csv
  python cli.py universal seed -f data/OI_Genetics_from_bill_modified_kept_genes.csv -d MONDO_0019019 $CLICKHOUSE_ARGS $NEO4J_ARGS

  python cli.py network update-disease-metadata -f data/ot_25.03_disease_mapping.csv $NEO4J_ARGS
  python cli.py network update-property-metadata -f data/property_description_tdp.csv $NEO4J_ARGS
} 2>&1 | tee -a "$LOGFILE"
```
