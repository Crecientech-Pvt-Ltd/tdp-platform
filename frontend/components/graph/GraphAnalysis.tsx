...
export function GraphAnalysis({
	...
	const [communityMap, setCommunityMap] = useState<
		Record<string, { name: string; genes: string[]; color: string }>
	>({});
	...
	const nodeDegreeProperty = useStore(
		(state) => state.radialAnalysis.nodeDegreeProperty,
	);
	const universalData = useStore((state) => state.universalData);

	const [fetchUniversal] = useLazyQuery<
		GeneUniversalData,
		GeneUniversalDataVariables
	...
	useEffect(() => {
		(async () => {
			let nodeCount = 0;
			const userOrCommonIdentifier = useStore
				.getState()
				.radioOptions.user.TE.includes(nodeDegreeProperty)
				? "user"
				: "common";
			...
			if (!isNodeDegree) {
				await fetchUniversal({
					...
					for (const gene of data?.genes ?? []) {
						...
						if (value) {
							universalData[gene.ID][userOrCommonIdentifier].TE[
								nodeDegreeProperty
							] = value;
							const num = +value;
							if (!Number.isNaN(num)) {
					...
					graph.updateEachNodeAttributes((node, attr) => {
						...
						if (
							!Number.isNaN(value) &&
							sizeScale(value) >= radialAnalysis.nodeDegreeCutOff
						) {
							nodeCount++;
							attr.hidden = false;
						} else {
			...
			const edgeCount = graph.reduceEdges(
				(count, _edge, _attr, _src, _tgt, srcAttr, tgtAttr) => {
	...
	async function renewSession() {
		const res = await fetch(
			`${envURL(process.env.NEXT_PUBLIC_BACKEND_URL)}/algorithm/renew-session`,
			{
				method: "POST",
	...
	useEffect(() => {
		eventEmitter.on(
			Events.ALGORITHM,
			async ({ name, parameters }: EventMessage[Events.ALGORITHM]) => {
				if (name === "None") {
					setCommunityMap({});
					...
					if (searchParams?.get("file")) {
						const louvain = await import("graphology-communities-louvain").then(
							(lib) => lib.default,
						);
						...
						const map: Record<
							string,
							{ name: string; genes: string[]; color: string }
						> = {};
						let count = 0;
					...
					(async function louvain() {
						...
						if (res.ok) {
							...
							toast.error("Failed to fetch Louvain data", {
								cancel: { label: "Close", onClick() {} },
								description:
									"Server not available,Please try again later. Graph must have relationships to run Louvain.",
							});
						}
					})();
				}
					...
					{Object.entries(communityMap).map(([id, val], idx) => (
						<div key={id} className="flex items-center gap-1">
							...
							<Button
								...
								className="h-5 w-30"
								onClick={() =>
									fitViewportToNodes(sigma, val.genes, { animate: true })
								}
							>
								Community {idx + 1}
							</Button>
