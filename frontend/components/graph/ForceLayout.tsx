...
export function ForceLayout() {
	...
	const simulation =
		useRef<Simulation<NodeAttributes, SimulationLinkDatum<NodeAttributes>>>();
	...
	useEffect(() => {
		...
		(sigma as EventEmitter).once("loaded", () => {
			...
			simulation.current = forceSimulation<
				NodeAttributes,
				SimulationLinkDatum<NodeAttributes>
			>(nodes.current)
				.force(
					"link",
					forceLink<NodeAttributes, SimulationLinkDatum<NodeAttributes>>(
						edges.current,
					)
						.id((d) => d.ID!)
