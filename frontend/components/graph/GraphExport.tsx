...
export function GraphExport({
	...
	useEffect(() => {
		eventEmitter.on(
			Events.EXPORT,
			({ format, all }: EventMessage[Events.EXPORT]) => {
				switch (format) {
					case "json": {
					...
					case "csv": {
						if (
							!all &&
							(!highlightedNodesRef || highlightedNodesRef.current.size === 0)
						) {
						...
						const isDatabaseOrUser = (
							radio: NodeColorType | NodeSizeType,
							property: string,
						) =>
							radioOptions.user[radio].includes(property)
								? "user"
								: DISEASE_DEPENDENT_PROPERTIES.includes(
											radio as DiseaseDependentProperties,
										)
									? diseaseName
									: "common";
						...
						const data = unparse(
							(all
								...
								: Array.from(highlightedNodesRef?.current!)
							).map((nodeId) => {
									...
									if (typeof selectedNodeColorProperty === "string") {
										...
										for (const property of selectedNodeColorProperty) {
											universalProperties[property] = (
												universalData[nodeId][
													isDatabaseOrUser(selectedRadioNodeColor, property)
												] as CommonSection & OtherSection
								...
								if (selectedRadioNodeSize) {
									if (typeof selectedNodeSizeProperty === "string") {
										...
										for (const property of selectedNodeSizeProperty) {
											universalProperties[property] = (
												universalData[nodeId][
													isDatabaseOrUser(selectedRadioNodeSize, property)
												] as CommonSection & OtherSection
