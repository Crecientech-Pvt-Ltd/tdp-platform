...
export function SizeAnalysis() {
	...
	useEffect(() => {
		...
		const isUserProperty =
			...
			radioOptions.user[selectedRadioNodeSize].includes(
		...
		if (
			selectedRadioNodeSize === "Druggability" &&
			typeof selectedNodeSizeProperty === "string"
		) {
			const minMax = Object.values(universalData).reduce(
				(acc, cur) => {
					const valString =
						cur[userOrCommonIdentifier].Druggability[selectedNodeSizeProperty];
			...
			const sizeScale = scaleLinear<number, number>(minMax, [
				3,
				defaultNodeSize + 10,
			]);
			...
			const minMax = Object.values(universalData).reduce(
				(acc, cur) => {
					const valString =
						cur[userOrCommonIdentifier].TE[selectedNodeSizeProperty];
					if (!valString) return acc;
			...
			selectedRadioNodeSize === "GWAS" &&
			typeof selectedNodeSizeProperty === "string"
		) {
			const minMax = Object.values(universalData).reduce(
				(acc, cur) => {
					const valString = (cur[userOrDiseaseIdentifier] as OtherSection)
						.GWAS?.[selectedNodeSizeProperty];
					if (!valString) return acc;
					const value = +valString;
					return [Math.min(acc[0], value), Math.max(acc[1], value)];
				},
			...
			selectedRadioNodeSize === "GDA" &&
			typeof selectedNodeSizeProperty === "string"
		) {
			const minMax = Object.values(universalData).reduce(
				(acc, cur) => {
					const valString = (cur[userOrDiseaseIdentifier] as OtherSection)
						.GDA?.[selectedNodeSizeProperty];
					if (!valString) return acc;
					const value = +valString;
					return [Math.min(acc[0], value), Math.max(acc[1], value)];
				},
