...
export function NodeSize({
	onPropChange,
}: { onPropChange: (prop: string | Set<string>) => void }) {
		...
		<Collapsible defaultOpen className="mb-2 border p-2 rounded shadow">
				...
				<div className="space-x-1 flex items-center">
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
								onClick={() =>
									useStore.setState({ selectedRadioNodeSize: undefined })
								}
								type="button"
								variant="outline"
								size="icon"
			...
			<CollapsibleContent className="mt-2">
				<RadioGroup
					...
					onValueChange={(value) =>
						useStore.setState({
							selectedRadioNodeSize: value as NodeSizeType,
						...
						return (
							<Tooltip key={label}>
								<div className="flex items-center space-x-2">
									...
									<TooltipTrigger asChild>
										{tooltipContent && <Info size={12} className="shrink-0" />}
									</TooltipTrigger>
								</div>
								{tooltipContent && (
				...
				{radioValue &&
					(radioValue === "TE" ? (
						<VirtualizedCombobox
							key={radioValue}
							data={[
								...radioOptions.database[radioValue],
								...radioOptions.user[radioValue],
							]}
							className="mt-2"
