  ...
  useEffect(() => {
    ...
    fetchHeader({
      ...
      .then(val => {
        ...
        // radioOptions.database.OpenTargets = data.openTargets ?? [];
        radioOptions.database.LogFC = data.differentialExpression ?? [];
        radioOptions.database.Genetics = data.genetics ?? [];
    ...
    if (properties.length === 0) {
      ...
      for (const { ID, data } of result.data?.geneProperties ?? []) {
        ...
        for (const { key, score, category: categoryEnum, diseaseId } of data) {
          ...
          if (diseaseId) {
            if (category !== 'LogFC' && category !== 'OpenTargets' && category !== 'Genetics') continue;
