...
const DISEASE_DEPENDENT_FIELDS = ["GWAS", "GDA", "LogFC", "Genetics"];
...
const argv = yargs(process.argv.slice(2))
	...
	.example(
		chalk.blue(
			"node $0 -f universal.csv -U bolt://localhost:7687 -u neo4j -p password -d pdnet -D ALS --nh",
		),
	)
	.example(
		chalk.blue(
			"node $0 -f universal.csv -U bolt://localhost:7687 -u neo4j -p password -d pdnet --di --nh",
		),
	)
...
(async () => {
	...
	if (disease) {
		console.info(
			...
			chalk.cyan(`Detected disease: ${disease}`),
		);
	}
	if (
		!file ||
    ...
    `.replace(/"/g, "");
		...
		try {
			...
			const indexQuery =
				"CREATE INDEX Gene_name_Gene IF NOT EXISTS FOR (g:Gene) ON (g.Gene_name)";
      ...
      `,
			...
			await session.run(
				`
      MERGE (d:Disease { name: $disease }) WITH d
      UNWIND $diseaseHeaders AS diseaseHeader
      MERGE (d)-[:HAS_PROPERTY]->(dp:Property { name: diseaseHeader, description: diseaseHeader });
      `,
				{
					disease,
					diseaseHeaders,
			...
			console.log(
				chalk.green(
					chalk.bold("[LOG]"),
					"Added Disease, Headers to Stats (If not already present)",
				),
			);
