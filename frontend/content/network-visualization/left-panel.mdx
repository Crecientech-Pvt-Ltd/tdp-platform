...
[Knowledge Base](../knowledge-base/index.mdx) integrated in our tool is build based on [disease-dependent data](../knowledge-base/disease-dependent-data/index.mdx) and [disease-independent data](../knowledge-base/disease-independent-data/index.mdx). Disease-dependent data indicates the data of specific features varies based on different diseases, while disease-independent data indicates the data remains the same once the disease changes.

Disease-dependent data includes [LogFC](../knowledge-base/disease-dependent-data/logfc.mdx), [GDA](../knowledge-base/disease-dependent-data/gda.mdx)and [Genetics](../knowledge-base/disease-dependent-data/genetics.mdx), while disease-independent data includes [Target Prioritization Factors](../knowledge-base/disease-independent-data/target-prioritization-factors.mdx), [Pathway](../knowledge-base/disease-independent-data/pathway.mdx), [Druggability](../knowledge-base/disease-independent-data/druggability.mdx), [Tissue Enrichment](../knowledge-base/disease-independent-data/tissue-enrichment.mdx). We also call these data types as **features**.
...
- Please note that only the [disease-dependent data](../knowledge-base/disease-dependent-data/index.mdx) will change if you switch to another disease.
...
1. Upload a **CSV** file with your own data, please refer to the [File Format](left-panel.mdx#file-format) below.
   > **Note**: We allow multiple files to be uploaded.
2. Click Submit.
3. Locate Node Color and/or Node Size section and select the corresponding uploaded feature name.
   > **Note**: The uploaded data column name will be shown in the dropdown list of Node Color and Node Size section, with the prefix **[User]**.
4. Select the column name of your customized data.

![Custom upload](../../public/image/docs/custom-upload.png)

If you don't need the customized data anymore, you can click the **Reset** button <Image width={30} height={30} src="/image/docs/reset_uploads.png" alt="Reset Uploads" className="inline aspect-square"/> to remove the data.
...
| ----------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| LogFC                         | LogFC\_<pre className="inline text-red-500">CustomName</pre>              | [-Inf, +Inf]                                                                                                |
| GDA                           | [GDA\|OpenTargets]\_<pre className="inline text-red-500">CustomName</pre> | [0, 1]                                                                                                      |
| Genetics                      | [GWAS\|Genetics]\_<pre className="inline text-red-500">CustomName</pre>   | [-1, 1]                                                                                                     |
| Target Prioritization Factors | OT_Prioritization\_<pre className="inline text-red-500">CustomName</pre>  | [0, 1]                                                                                                      |
...
| Custom                        | Custom_Color\_<pre className="inline text-red-500">CustomName</pre>       | Any hex color code or CSS color name, see list [here ↗](https://www.w3schools.com/colors/colors_names.asp) |
...
You can export the network you created. Please refer to the [Network Export](network-visualization-board.mdx#network-export) section in the [Network Visualization Board](network-visualization-board.mdx) page for more details.
